...
def play_game():
    if request.method == "POST":
        ...
        with redirect_stdout(output):
            print("Game started!")
            # Use WebInputParser to automate the choices
            parser = WebInputParser(max_party_size=5)
            ...
            game = Game(
                parser, total_characters, locations, chosen_party, opposing_team
            )
            game.start()  # Actually start the game
        ...
        if is_ajax:
            return jsonify({"game_output": terminal_output})
        ...
        <body>
            <h1>Game Output</h1>
            <pre>{{ output }}</pre>
            <a href="/"><button class="back-button">Play Again</button></a>
        </body>
        </html>
        """
        return render_template_string(html_template, output=terminal_output)
    else:
        # If GET request, show the game start page
        return render_template("index.html")
...
class Character:
    def __init__(
        self,
        name: str = "Bob",
        strength_value: int = 10,
        intelligence_value: int = 10,
        stamina_value: int = 10,
        agility_value: int = 10,
    ):
        self.name = name
        ...
        self.strength = Statistic(
            ...
            description="Strength is a measure of physical power.",
        )
        self.intelligence = Statistic(
            "Intelligence",
            value=intelligence_value,
            description="Intelligence is a measure of cognitive ability.",
        )
        self.stamina = Statistic(
            "Stamina",
            value=stamina_value,
            description="Stamina is a measure of endurance",
        )
        self.agility = Statistic(
            "Agility",
            value=agility_value,
            description="Agility is a measure of nimbleness",
        )
    ...
    def get_stats(self):
        return [
            ...
            self.stamina,
            self.agility,
        ]  # Extend this list if there are more stats
...
class Event:
    ...
    def resolve_choice(self, character: Character, chosen_stat: Statistic, game):
        if chosen_stat.name == self.primary_attribute:
            ...
            if (
                random.random() < 0.5
            ):  # 50% chance to let them try again with another stat
                print(
                    "You nearly made a mistake, but you're given the opputunity to try again..."
                )
...
class Game:
    def __init__(
        ...
        self.completed_events = (
            0  # Track how many events have been completed in the current location
        )
    ...
    def display_party(self):
        ...
        for member in self.party:
            print(
                f"{member.name} - Strength: {member.strength.value}, Intelligence: {member.intelligence.value}, Stamina: {member.stamina.value}, Agility: {member.agility.value}"
            )
    ...
    def move_to_new_location(self):
        ...
        next_location = self.locations[
            (self.locations.index(self.current_location) + 1) % len(self.locations)
        ]
    ...
    def trigger_special_event(self):
        ...
        surprise_events = [
            {
                "description": "You stepped on a hidden trap! Lose 10 stamina.",
                "effect": lambda player: player.stamina.modify(-10),
            },
            {
                "description": "You find a Speed Boost potion! Gain +10 agility.",
                "effect": lambda player: player.agility.modify(10),
            },
            {
                "description": "A spiked pit appears! Lose 5 agility escaping it.",
                "effect": lambda player: player.agility.modify(-5),
            },
            {
                "description": "You find a Strength Gauntlet! Gain +15 strength.",
                "effect": lambda player: player.strength.modify(15),
            },
            {
                "description": "A sudden rockslide hits you! Lose 10 stamina.",
                "effect": lambda player: player.stamina.modify(-10),
            },
            {
                "description": "You discover an Invisibility Cloak! Sneak past obstacles next round.",
                "effect": lambda _: self.activate_invisibility(),
            },
            {
                "description": "A lightning strike charges you up! Gain +5 strength and +5 agility.",
                "effect": lambda player: (
                    player.strength.modify(5),
                    player.agility.modify(5),
                ),
            },
            {
                "description": "An enemy sniper takes a shot! One random teammate loses 15 stamina.",
                "effect": lambda _: self.random_team_damage(15),
            },
            {
                "description": "You find a Medkit! Restore 20 stamina.",
                "effect": lambda player: player.stamina.modify(20),
            },
            {
                "description": "An adrenaline rush hits you! Gain +10 stamina and +10 agility.",
                "effect": lambda player: (
            ...
            {
                "description": "A bear suddenly appears! Lose 15 stamina while running away.",
                "effect": lambda player: player.stamina.modify(-15),
            },
            {
                "description": "You find an Ancient Amulet! ALL your stats increase by +5.",
                "effect": lambda player: (
                    player.strength.modify(5),
                    player.intelligence.modify(5),
                    player.stamina.modify(5),
                    player.agility.modify(5),
                ),
            },
            {
                "description": "You fall into a pit trap! Lose 5 agility and 5 stamina.",
                "effect": lambda player: (
                    player.agility.modify(-5),
                    player.stamina.modify(-5),
                ),
            ...
            {
                "description": "You drink mysterious water... Gain +10 intelligence!",
                "effect": lambda player: player.intelligence.modify(10),
            },
        ]
        ...
        if "Invisibility" in selected_event["description"]:
            selected_event["effect"](
                None
            )  # Invisibility affects the team, not individual players
    ...
    def random_team_damage(self, damage_amount):
        if self.party:
            ...
            if unlucky_player.stamina.value <= 0:
                print(
                    f"💀 {unlucky_player.name} has collapsed from exhaustion and is out of the game!"
                )
    ...
    def trigger_mini_boss(self, boss_number=1):
        ...
        if total_team_strength > boss_strength:
            print(
                "🎉 You defeated the Mini-Boss and earned +10 stamina and +10 agility for everyone!"
            )
            for player in self.party:
                player.stamina.modify(10)
                player.agility.modify(10)
            ...
            print(
                "💀 The Mini-Boss overpowered your team! You lose one random team member."
            )
            if self.party:
...
class UserInputParser:
    ...
    def make_your_party(
        self, total_characters: List[Character]
...
class WebInputParser(UserInputParser):
    def __init__(self, max_party_size: int):
        super().__init__(max_party_size)
        self.party = []
        self.max_party_size = max_party_size

    def parse(self, prompt: str) -> str:
        # Instead of waiting for user input, generate a reasonable choice
        print(f"[AUTO] Prompt was: {prompt}")
        return "1"  # Always choose the first option for simplicity

    def make_your_party(
        self, total_characters: List[Character]
    ) -> Tuple[List[Character], List[Character]]:
        print("Automatically selecting your party...")
        unchosen_characters = total_characters.copy()
        # Select first 'max_party_size' characters automatically
        self.party = total_characters[: self.max_party_size]
        print(
            f"Selected party members: {', '.join([char.name for char in self.party])}"
        )

        for char in self.party:
            if char in unchosen_characters:
                unchosen_characters.remove(char)
        return self.party, unchosen_characters

    def select_party_member(self, party: List[Character]) -> Character:
        # Select a random party member to create more interesting gameplay
        selected = random.choice(party)
        print(f"[AUTO] Selected party member: {selected.name}")
        return selected

    def select_stat(self, character: Character) -> Statistic:
        # Select a random stat to create more interesting gameplay
        stats = character.get_stats()
        selected_stat = random.choice(stats)
        print(f"[AUTO] Selected stat: {selected_stat.name} ({selected_stat.value})")
...
def start_game():
    ...
    # Create locations with their respective events
    locations = [Location(recon_events), Location(mid_events), Location(final_events)]
